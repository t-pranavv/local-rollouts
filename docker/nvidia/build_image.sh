...
torch_version="2.7.1"
transformer_engine_version="2.4"
...
# Infers current date and NVIDIA version from the base image
current_date=$(date +"%Y%m%d")
nvidia_version=$(echo "$base_image" | sed -E 's|.*/pytorch:([0-9]+\.[0-9]+)-.*|\1|')

# Builds the custom image
output_image_tag="aifrontiers.azurecr.io/nvidia${nvidia_version}-pytorch${torch_version}-te${transformer_engine_version}-deepspeed${deepspeed_version}-flashattn${flash_attention_version}${vllm_tag}:${current_date}"
docker build . \
    --file Dockerfile \

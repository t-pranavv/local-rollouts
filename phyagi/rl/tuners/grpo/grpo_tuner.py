...
from phyagi.rl.tuners.grpo.grpo_worker import RayGRPOWorker
...
class RayGRPOTuner:
    ...
    def __init__(
        self,
        args: Optional[RayGRPOConfig] = None,
    ...
    def _init_workers(self) -> None:
        worker_cls = ray.remote(num_gpus=1.0)(RayGRPOWorker)
        worker_kwargs = dict(
            config=self.args,
        ...
        build_reference_model = self.args.kl_coeff > 0
        ray.get([worker.configure_models.remote(build_reference_model) for worker in self.workers])
    ...
    def get_train_dataloader(self, total_consumed_samples: int = 0, shuffle: bool = True) -> DataLoader:
        ...
        # Since `RayGRPOTuner` runs on a single process, we manually set the rank to 0
    ...
    def generate_completions(
        ...
        **generation_kwargs,
        ...
        # Distribute and possibly repeat data for worker TP groups
    ...
    def save_checkpoint(self, output_dir: Union[str, Path], overwrite_reference_model: bool = False) -> None:
        """Save the current state of the tuner.

        Args:
            output_dir: Directory to save the checkpoint.
            overwrite_reference_model: Whether to overwrite the reference model if it already exists.
        ...
        if self.args.kl_coeff > 0:
            ...
            if overwrite_reference_model or not reference_dir.exists():
                reference_dir.mkdir(exist_ok=True, parents=True)
                ray.get([worker.save_reference_model.remote(reference_dir) for worker in self.workers])
    ...
    def load_checkpoint(
        ...
        if self.args.kl_coeff > 0:
            if not (load_dir / "reference").exists():
                raise FileNotFoundError(f"'{load_dir}' does not contain a reference model.")
            ray.get([worker.load_reference_model.remote(load_dir / "reference") for worker in self.workers])
    ...
    def evaluate(self) -> Dict[str, Any]:
        ...
        # Shuffle dataset to better distribute data sources across workers
        ...
        eval_completions = self.generate_completions(
            prompt_input_ids,
            num_repetitions=self.args.eval_group_size or self.args.group_size,
            batch_size=self.args.eval_batch_size,
            sync_weights=True,
            detokenize=True,
            use_tqdm=True,
            **self.args.rollout.eval_sampling_params,
        )
    ...
    def train(
        ...
        for step in tqdm(range(self.global_step, self.training_steps + 1)):
            ...
            if self._is_eval_step(step):
                self.logger.info(f"[step={step}] Evaluating model...")
            ...
            with timer.measure("timing/actor_update"):
                actor_metrics = self._update_actor_policy(
                    packed_batches, temperature=self.args.rollout.sampling_params["temperature"]
                )
            ...
            self.logger.info(
                {
                    ...
                    **eval_reward_metrics,
                    **timer.metrics(total_key="timing/grpo_step", end=True),
                }
            )

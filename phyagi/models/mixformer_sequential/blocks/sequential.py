...
class SequentialBlock(nn.Module):
    """Sequential block (used in GPT).

    This block applies sequential mixer and MLP layers to the input (used in GPT).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ln_1 = get_norm(config.n_embd, norm_config=norm, eps=config.layer_norm_epsilon)
        self.ln_2 = get_norm(config.n_embd, norm_config=norm, eps=config.layer_norm_epsilon)

        self.attn = get_mixer(config, mixer_config=mixer, mixer_idx=block_idx)
    ...
    def set_torch_tp(self, tp_mesh: DeviceMesh, enable_sequence_parallel: bool = False, **kwargs) -> None:
        if tp_mesh.size() > 1:
            ...
            if hasattr(self.attn, "set_torch_tp"):
                plan.update(
                    {

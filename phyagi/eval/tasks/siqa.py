...
class SIQA:
    ...
    def mapping_fn(example: Dict[str, Any]) -> List[Dict[str, Any]]:
        prompt = "{}\nQuestion: {}\nAnswer:{}"
        targets = [" " + example["answerA"], " " + example["answerB"], " " + example["answerC"]]

        return [
            {
                "text": prompt.format(example["context"], example["question"], target),
                "target": target,
    ...
    def run(
        ...
        **kwargs,
        ...
        dataset = load_dataset(SIQA.DATASET_PATH, name=SIQA.DATASET_NAME)["validation"]
        ...
        responses = generate(
            ...
            tokenizer=tokenizer,
            device=device,
            example_generator_kwargs={"mapping_fn": SIQA.mapping_fn},
            **kwargs,
        )

...
BENCHMARKS = {"aime_2024": 30, "aime_2023": 30, "aime_2022": 30, "gpqa_diamond": 198, "math_500_test": 500}
...
def process_jsonl_results(result_folder, num_expected_problems):
    ...
    for result in results:
        ...
        agg_scores = result["aggregated_score"]
    ...
    accuracy_per_sample = [np.mean([s[i] for s in metrics["all_scores"]]) for i in range(len(metrics["all_scores"][0]))]
...
def evaluate_models(output_dir, temperature):
    ...
    for benchmark in BENCHMARKS:
        ...
        for eval_type in EVAL_TYPES[benchmark]:
            ...
            result_folder = os.path.join(output_dir, f"{benchmark}", eval_type)

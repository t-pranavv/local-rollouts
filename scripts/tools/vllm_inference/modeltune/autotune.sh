...
BASE=${EXPERIMENT_OUTPUT_DIR:-output}
MODEL=${MODEL_PATH:-"Qwen/Qwen3-32B"}
TOKENIZER=${TOKENIZER_PATH:-"Qwen/Qwen3-32B"}
SYSTEM="GPU"
TP=${TENSOR_PARALLEL_SIZE:-1}
INPUT_LEN=${MAX_INPUT_LEN:-4096}
OUTPUT_LEN=${MAX_OUTPUT_LEN:-256}
MIN_CACHE_HIT_PCT=0
...
while (( $(echo "$gpu_memory_utilization >= 0.78" | bc -l) )); do
    ...
    if [[ "$result" -eq 0 ]]; then
        ...
        gpu_memory_utilization=$(echo "$gpu_memory_utilization - 0.02" | bc)
    fi
done
...
if [[ "$find_gpu_memory_utilization" -eq 1 ]]; then
    ...
    echo "Cannot find a proper gpu_memory_utilization over 0.8 to serve the model, please check logs in $LOG_FOLDER."
    exit 1
fi
